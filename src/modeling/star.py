...
class STARModel(T5ForConditionalGeneration):

    def __init__(self, config: T5Config, num_masked_tokens=1):
    ...
    def forward(
        ...
        lm_logits[:, :, :self.num_masked_tokens] = -float("inf")

        """
        for i in range(lm_logits.shape[0]):
            for j in range(lm_logits.shape[1]):
                lm_logits[i, -1, decoder_input_ids[i, j]] = -float("inf")"""
